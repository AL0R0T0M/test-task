...
class WalletAPITests(APITestCase):
    def setUp(self):
        self.wallet = Wallet.objects.create(
            label="Test Wallet", balance=Decimal("100.00"))
        self.get_url = reverse(
            'wallet-detail', kwargs={'wallet_uuid': self.wallet.id})
        self.operation_url = reverse(
            'wallet-operation', kwargs={'wallet_uuid': self.wallet.id})
    ...
    def test_get_wallet_balance(self):
        ...
        self.assertEqual(
            Decimal(response.data['balance']), self.wallet.balance)
